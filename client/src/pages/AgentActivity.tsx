...
export default function AgentActivity() {
  ...
  const filteredChartData = useMemo(() => {
    if (timeRange === 'Max') {
      return agentChartData; // Show all data for Max
    }
    ...
    const getDataPointCount = (range: TimeRange): number => {
      switch (range) {
        ...
        case '1Y': return Math.min(12, agentChartData.length); // Last year (capped at available data)
        default: return agentChartData.length;
      }
    };
    ...
    return agentChartData.slice(-pointCount);
  }, [agentChartData, timeRange]);
